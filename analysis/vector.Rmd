...
It contains geographical data in lat/lot decimal degrees. We can convert the sampling locations to `sf` geographic points using function `sf::st_as_sf()`. We can also assign a CRS, in this case WGS 84 which corresponds to epsg:4326.
...
### Plotting the study area location
...
p + 
    geom_polygon(data = mx_coords, aes(x = X, y = Y), colour = "black", fill = "lightgrey") +
    geom_polygon(data = bbox_coords, aes(x = X, y = Y), colour = "black", fill = "white") +
    coord_quickmap()
...
p + 
    geom_polygon(data = mx_utm15_coords, aes(x = X, y = Y), colour = "black", fill = "lightgrey") +
    geom_polygon(data = bbox_coords, aes(x = X, y = Y), colour = "black", fill = "white") +
    coord_quickmap()
...
### `geom_sf` vs `geom_polygon` summary


- When using `geom_sf` ggplot2 **automatically converts all objects in a plot to the CRS of the first plot layes**. This is **not the case with `geom_polygon`** so need to make sure coordinates of data from different sources are in the same CRS.
- Without `coord_quickmap()`, ggplot will not automatically adjust the window to reflect the mapped geography allowing for potential distortion.
...
Let's also write out our study bounding box for use in the next section.

```{r}
write_sf(study_bbox, here::here("data", "sf", "study_bbox.geojson"))
```
