...
This bounding box we originally defined is really tight around our data points. To ensure our raster data contain the locations of all our data points, let's give it some space around our points by using function `sf::st_buffer`. 
...
Let's use this to calculate the deviation of our datapoints from the study box mean. We can use `dplyr::mutate` to manipulate attribute data in our `sf` just like any other data.frame.
